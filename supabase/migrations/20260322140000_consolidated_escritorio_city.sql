/*
 * Escritorio City — UNICA migracao consolidada (cole no SQL Editor ou supabase db push).
 *
 * Inclui: pgcrypto, public.offices, public.users + trigger auth, RLS, roles escritorio_*,
 * grants por coluna via roles, seed demo (on conflict slug ignora).
 *
 * Se trigger falhar: troque "execute function" por "execute procedure".
 *
 * Projeto ja com public.offices antigo: prefira backup; esta migracao usa CREATE TABLE IF NOT EXISTS
 * e nao adiciona colunas novas a tabelas legadas (use ALTER manual se precisar).
 */
